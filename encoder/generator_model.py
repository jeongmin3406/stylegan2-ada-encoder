...
class Generator:
    def __init__(self, model, batch_size, randomize_noise=False):
        ...
        in_expr, out_expr = next(expr for expr in model.components.synthesis._run_cache.values())

        self.dlatent_variable = next(v for v in in_expr if 'learnable_dlatents' in v.name)
        self.generator_output = next(v for v in out_expr if '_Run/concat' in v.name)
    ...
    def set_dlatents(self, dlatents):
        assert (dlatents.shape == (self.batch_size, 14, 512))
